//
//  UICollectionView+LayoutCell.swift
//  ListViewAutoHeightExtention
//
//  Created by nothot on 2021/3/31.
//
...
extension UICollectionView {
    ...
    var cellDic: [String: UICollectionViewCell] {
        get {
            let value = objc_getAssociatedObject(self, &AssociatedKeys.cellDicKey) as? [String: UICollectionViewCell] ?? [:]
            if value.isEmpty {
    ...
    func dequeueLayoutCell(identifier: String, indexPath: IndexPath = IndexPath(item: 0, section: 0)) -> UICollectionViewCell {
        
        var cell: UICollectionViewCell
        if let cachedCell = cellDic[identifier] {
            cell = cachedCell
        }else {
            cell = dequeueReusableCell(withReuseIdentifier: identifier, for: indexPath)
            cellDic[identifier] = cell
        }
        return cell
    }
    
    public func sizeForItem<T>(identifier: String, indexPath: IndexPath = IndexPath(item: 0, section: 0), cacheByKey: T, configuration: (UICollectionViewCell) -> Void) -> CGSize  where T: Hashable {
    ...
    func fittingSize(for configuratedCell: UICollectionViewCell) -> CGSize {
